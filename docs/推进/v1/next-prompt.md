...
- 下一最小节点已确定为 `M7 Canvas Spike`：先做 Task 节点投影、节点选中与和现有任务区联动。
...
接下来请优先推进 `M7 Canvas Spike`，并优先补齐“能看懂结构、能中断后快速接上、能继续按人工验证闸门推进”的部分。

建议的新阶段目标：
- 明确 `Task / Executor / TerminalSession / Project` 的关系
- 明确 canvas / 看板 / 结构层的最小数据模型
...
- `M7 Canvas Spike` 当前只验证主界面方向，不要直接扩成完整白板编辑器
