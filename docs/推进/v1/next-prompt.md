...
- 网页终端最小闭环已通过人工验证：可创建、可输入输出、支持 resize、支持刷新恢复。
- 执行器注入最小样例也已通过人工验证：可把预设命令注入当前终端执行。
- 执行器单参数输入也已通过人工验证：文件名会影响命令预览与最终生成文档名。
- 执行器参数输入也已通过人工验证：文件名可驱动命令预览与最终生成文件名变化。
...
接下来请继续推进 v1 的下一最小节点，并优先补齐“能运行、能交互、能验证”的部分。当前优先项是执行器状态栏。
